...
play game@Game {gameBoard} (Fund player chain coord) =
  if gameBoard `hasNeutralChainAt` coord
...
placeTile game@Game {..} name coord =
  let isTilePlayable = find ((== name) . playerName) (M.elems players) >>= find (== coord) . tiles
...
drawTile name (Just tile) game@Game {..}
  ...
  | otherwise =
    let removeTile t p = p {tiles = head drawingTiles : delete t (tiles p)}
     in game
...
doPlayTile _ Nothing game = game
doPlayTile name (Just tile) game@Game {..} =
  let newCell = Cell tile (Neutral tile)
      adj = linkedCells gameBoard newCell
      owners = nub $ sort $ mapMaybe (isOwned . cellContent) adj
   in case owners of
        [] ->
          game
            { gameBoard = gameBoard // [(tile, newCell)],
              turn =
                if hasAdjacentNeutralTile gameBoard tile
...
tileExpandsExistingChain chainName adj name game@Game {..} =
  game
    { gameBoard = gameBoard // map (\(Cell t _) -> (t, Cell t (Chain chainName))) adj,
...
createNewChain game@Game {..} player chain coord =
  let linked = linkedCells gameBoard (Cell coord (Neutral coord))
   ...
   in game
        { gameBoard = gameBoard // map (\(Cell t _) -> (t, (Cell t (Chain chain)))) linked,
...
buyStock game@Game {..} player chain =
  let price = stockPrice (hotelChains M.! chain)
   ...
   in if hasEnoughMoneyToBuyStock (players M.! player) (hotelChains M.! chain)
        ...
        then
          game
            { hotelChains = M.adjust decreaseStock chain hotelChains,
        ...
        then
          game
            { turn =
                ( name,
                  ResolveMerger
                    (TakeOver tile [largestChain, secondChain])
...
computeMergerBonus game@Game {..} chain =
  let buyeeChain = hotelChains M.! chain
   ...
   in case shareHolders of
        ...
        (fsts : snds : _) ->
          divideAmong (map fst snds) (snd $ mergerBonus buyeeChain) $
...
merge game@Game {..} _ tile buyer buyee =
  let buyerChain = hotelChains M.! buyer
   ...
   in if length (chainTiles buyerChain) >= length (chainTiles buyeeChain)
        ...
        then
          game'
            { hotelChains = M.adjust clearBuyee buyee $ M.adjust mergeIntoBuyer buyer hotelChains,
              gameBoard = gameBoard // map (\t -> (t, (Cell t (Chain buyer)))) mergedTiles,
              turn =
                ( head $ M.keys players,
                  let (_, ResolveMerger _ cont) = turn
                   in ResolveMerger
                        (DisposeStock (nextPlayer game) buyer buyee (stockPrice buyeeChain) (M.keys players))
                        cont
                )
            }
        else game
...
sellStock game@Game {..} player chain1 price qty =
  case stockLookup chain1 (ownedStock $ players M.! player) of
    ...
    Just {} ->
      let soldStock p =
            p
              { ownedStock = adjustStock (\q -> q - qty) chain1 (ownedStock p),
                ownedCash = ownedCash p + price * qty
              }
          increaseStock c = c {chainStock = chainStock c + qty}
       in game
            { hotelChains = M.adjust increaseStock chain1 hotelChains,
...
exchangeStock game@Game {..} player buyer buyee qty =
  case stockLookup buyee (ownedStock $ players M.! player) of
    Nothing -> game {turn = nextTurnInMergerSolving game turn}
    Just {} ->
      let buyerRemainingStock = chainStock $ hotelChains M.! buyer
          xchgedStock = min buyerRemainingStock qty
          xchgStock p =
            p
              { ownedStock =
                  adjustStock (+ xchgedStock) buyer $
       ...
       in game
            { hotelChains = M.adjust decreaseStock buyer $ M.adjust increaseStock buyee hotelChains,
...
endGame game@Game {..} =
  let game' = foldl computeMergerBonus game (map chainName $ activeChains hotelChains)
   ...
   in game'
        { players = M.map sellEverything players,
