...
possiblePlay game@(Game _ _ _ _ chains (name, FundChain t))    =  let availableChainsForFounding = (filter (not . hasActiveChain game) $ M.keys chains)
                                                                in if not (null availableChainsForFounding)
                                                                   then completeWithEndGame chains $
                                                                        map (\ c -> Fund name c t) availableChainsForFounding
                                                                   else [Pass]

possiblePlay game@(Game _ _ plys _ chains (name, BuySomeStock _)) =  completeWithEndGame chains $
